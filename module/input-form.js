...
export function createTaxResidentStep(textConfig) {
  ...
  step.innerHTML = html`<div class="step-title-row"><h2>${cfg.title || 'Status'}</h2><span class="question-icon" tabindex="0"><img src="asset/question_icon.webp" alt="info" /><span class="info-box">${tooltip}</span></span></div><select id="tax-resident-status"><option value="" disabled selected>${cfg.selectPlaceholder || 'Select...'}</option><option value="local">${(cfg.options && cfg.options.local) || 'Local'}</option><option value="expat">${(cfg.options && cfg.options.expat) || 'Expat'}</option></select>`;
  return step;
}
...
export function createSalaryStep(textConfig, salaryType) {
  ...
  const warning = (textConfig.warnings && textConfig.warnings.maxDigits) || stepCfg.warningMaxDigits || 'Maximum digits reached';
  const step = createElement('div', { className: 'form-step', id: 'step-2' });
  step.innerHTML = html`<div class="step-title-row"><h2>${stepCfg.title || 'Base Salary'}</h2><span class="question-icon" tabindex="0"><img src="asset/question_icon.webp" alt="info" /><span class="info-box">${tooltip}</span></span></div><input type="text" class="number-input" id="${inputId}" placeholder="${stepCfg.placeholder || ''}" /><div id="${inputId}-warning" class="input-warning hidden-initial">${warning}</div>`;
  return step;
}
...
export function createAllowanceStep(textConfig) {
  ...
  const warning = (textConfig.warnings && textConfig.warnings.maxDigits) || allowanceCfg.warningMaxDigits || 'Maximum digits reached';
  const keys = ['lunch', 'fuel', 'phone', 'travel', 'uniform', 'other'];
  step.innerHTML = html`<div class="step-title-row"><h2>${allowanceCfg.title || 'Allowance'}</h2><span class="question-icon" tabindex="0"><img src="asset/question_icon.webp" alt="info" /><span class="info-box">${tooltip}</span></span></div><div id="allowance-inputs"><div id="allowance-warning" class="input-warning hidden-initial">${warning}</div>${keys.map(k => html`<label class="input-label">${(allowanceCfg.types && allowanceCfg.types[k]) || k}<span class="question-icon" tabindex="0"><img src="asset/question_icon.webp" alt="info" /><span class="info-box">${(textConfig.infoTooltips && textConfig.infoTooltips[k]) || (allowanceCfg.tooltips && allowanceCfg.tooltips[k]) || ''}</span></span></label><input type="text" class="number-input" id="allowance-${k}" placeholder="${(allowanceCfg.placeholders && allowanceCfg.placeholders[k]) || ''}" min="0" />`).join('')}</div>`;
  return step;
}
...
export function createBonusStep(textConfig) {
  ...
  const warning = (textConfig.warnings && textConfig.warnings.maxDigits) || bonusCfg.warningMaxDigits || 'Maximum digits reached';
  step.innerHTML = html`<div class="step-title-row"><h2>${bonusCfg.title || 'Bonus'}</h2><span class="question-icon" tabindex="0"><img src="asset/question_icon.webp" alt="info" /><span class="info-box">${tooltip}</span></span></div><div id="bonus-inputs"><div id="bonus-warning" class="input-warning hidden-initial">${warning}</div><label class="input-label">${bonusCfg.totalBonusLabel || bonusCfg.title || 'Total Bonus'}</label><input type="text" class="number-input" id="total-bonus" placeholder="${bonusCfg.placeholder || ''}" /></div>`;
  return step;
}
...
export function createBenefitStep(textConfig) {
  ...
  const warning = (textConfig.warnings && textConfig.warnings.maxDigits) || benefitCfg.warningMaxDigits || 'Maximum digits reached';
  step.innerHTML = html`<div class="step-title-row"><h2>${benefitCfg.title || 'Benefit'}</h2><span class="question-icon" tabindex="0"><img src="asset/question_icon.webp" alt="info" /><span class="info-box">${tooltip}</span></span></div><div id="benefit-inputs"><div id="benefit-warning" class="input-warning hidden-initial">${warning}</div><label class="input-label">${(benefitCfg.types && benefitCfg.types.childTuition) || 'Child Tuition'}</label><input type="text" class="number-input" id="benefit-child-tuition" placeholder="${(benefitCfg.placeholders && benefitCfg.placeholders.childTuition) || ''}" /><label class="input-label">${(benefitCfg.types && benefitCfg.types.rental) || 'Rental'}</label><input type="text" class="number-input" id="benefit-rental" placeholder="${(benefitCfg.placeholders && benefitCfg.placeholders.rental) || ''}" /><label class="input-label">${(benefitCfg.types && benefitCfg.types.healthInsurance) || 'Health Insurance'}</label><input type="text" class="number-input" id="benefit-health-insurance" placeholder="${(benefitCfg.placeholders && benefitCfg.placeholders.healthInsurance) || ''}" /></div>`;
  return step;
}
...
export function initStandardForm({
  ...
  return { root, form, steps, nav, detachFormatting };
}
